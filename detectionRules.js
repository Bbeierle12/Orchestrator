// Enhanced file detection rules system
...
export const PROJECT_DETECTORS = [
  {
    ...
    check: (item, children) => {
      ...
      return children?.some(child => child.name === '.git');
    },
  ...
  },
  {
    ...
    check: (item, children) => {
      if (item.type !== 'folder') return false;
      return children?.some(child => child.name === 'package.json');
    },
  ...
  },
  {
    ...
    check: (item, children) => {
      ...
      return children?.some(child => indicators.includes(child.name));
    },
  ...
  },
  {
    ...
    check: (item, children) => {
      if (item.type !== 'folder') return false;
      return children?.some(child => child.name === 'Assets' && child.type === 'folder') &&
             children?.some(child => child.name === 'ProjectSettings');
    },
  ...
  },
  {
    ...
    check: (item, children) => {
      if (item.type !== 'folder') return false;
      return children?.some(child => child.name === 'Cargo.toml');
    },
  ...
  },
  {
    ...
    check: (item, children) => {
      if (item.type !== 'folder') return false;
      return children?.some(child => child.name === 'go.mod');
    },
    category: 'PROJECT_GO',
    reason: 'Go Project'
  },
  {
    ...
    check: (item, children) => {
      if (item.type !== 'folder') return false;
      return children?.some(child => child.name === 'pom.xml' || child.name === 'build.gradle');
    },
...
export const FOLDER_RULES = [
  {
    check: (item) => item.type === 'folder' && item.children?.length === 0,
    category: 'VACUUM_EMPTY',
    reason: 'Empty Folder'
  },
  {
    check: (item) => item.type === 'folder' && item.name.includes('temp') && (item.age || 0) > 30,
    category: 'VACUUM_TEMP',
    reason: 'Old Temp Folder'
  },
  {
    check: (item) => item.type === 'folder' && (item.age || 0) > 365 && !item.name.startsWith('.'),
    category: 'ARCHIVE_STALE',
    reason: `Inactive (${item.age}d)`
  },
  {
    check: (item) => item.name === '.dotnet',
    category: 'SDK_DOTNET',
    reason: '.NET SDK'
  },
  {
    check: (item) => item.name.startsWith('.') && item.type === 'folder',
...
export function detectFileCategory(item, children = null) {
  // Priority 1: Project detection (highest priority)
  for (const detector of PROJECT_DETECTORS) {
    if (detector.check(item, children)) {
      return {
